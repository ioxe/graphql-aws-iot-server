...
describe('Execution', () => {
    ...
    describe('Queries', () => {
        it('produces the expected AWS Iot Publish params for a successful query', done => {
                ...
                payload: `
                    {
                        ...
                        {"data":{"todos":[{"id":"1","name":"Todo 1","content":"Todo 1 Content"},
                        {"id":"2","name":"Todo 2","content":"Todo 2 Content"}]}}
                    }`,
                qos: 0,
            };
            ...
            subscriptionManager.onMessage(queryMessage, clientId, {})
                .then(res => {
                    const msgs = subscriptionManager.iotData.messages;
                    expect(msgs.length).toBe(2);
                    expect(msgs[0].topic).toEqual(expectedDataParams.topic);
                    expect(JSON.parse(msgs[0].payload)).toEqual(JSON.parse(expectedDataParams.payload));
                    expect(msgs[1].topic).toEqual(expectedCompleteParams.topic);
                    expect(JSON.parse(msgs[1].payload)).toEqual(JSON.parse(expectedCompleteParams.payload));
                    done();
                });
        });
    ...
    describe('Subscriptions', () => {
        ...
        it('returns expected AWS Iot Publish params for a subscription that does not exist', done => {
            const expectedErrorMessageParams = {
                ...
                payload: `{
                    ...
                    {"message":[
                        {"message":"Cannot query field \\"todoDoesntExist\\" on type \\"Subscription\\".",
            ...
            subscriptionManager.onMessage(queryMessage, clientId, {})
                .then(res => {
                    ...
                    expect(msgs[0].topic).toEqual(expectedErrorMessageParams.topic);
                    expect(JSON.parse(msgs[0].payload)).toEqual(JSON.parse(expectedErrorMessageParams.payload));
                    done();
                });
        });
        ...
        it('calls unsubscribe function and removeSubscriptionFn when a valid stop message is received', done => {
            const expectedErrorMessageParams = {
                topic: 'TEST/in/11231241',
                payload: `{"type":"error","id":"1","payload":
                    {"message":[{"message":"Cannot query field \\"todoDoesntExist\\" on type \\"Subscription\\".",
                    "locations":[{"line":1,"column":26}]}]}}`,
                qos: 0,
            };
